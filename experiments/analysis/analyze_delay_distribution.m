...
%% Figure: 시나리오별 통합 Boxplot
figure('Name', 'Delay Boxplot Combined', 'Position', [50 200 1800 600]);
...
%% 저장 (PDF + PNG)
exportgraphics(gcf, fullfile(output_dir, 'delay_boxplot_all.pdf'), 'ContentType', 'vector', 'Padding', 'figure');
exportgraphics(gcf, fullfile(output_dir, 'delay_boxplot_all.png'), 'Resolution', 600, 'Padding', 'figure');

fprintf('\n[저장 완료] %s/delay_boxplot_all.pdf / .png\n', output_dir);
...
function draw_boxplot(x, data, color, width)
    ...
    % 수염 (P10 ~ Q1, Q3 ~ P90)
    plot([x, x], [p10, q1], 'k-', 'LineWidth', 1);
    plot([x, x], [q3, p90], 'k-', 'LineWidth', 1);
    
    % 수염 끝 가로선
    cap_w = width / 4;
    plot([x-cap_w, x+cap_w], [p10, p10], 'k-', 'LineWidth', 1);
    plot([x-cap_w, x+cap_w], [p90, p90], 'k-', 'LineWidth', 1);
end
